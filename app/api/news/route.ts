...
const CATEGORY_TO_CURRENTS: Record<string, string> = {
  India: "general", World: "general", Local: "society", Business: "economy_business_finance",
  Technology: "science_technology", Entertainment: "arts_culture_entertainment", Sports: "sport",
  Science: "science_technology", Health: "health",
};
const CURRENTS_TO_CATEGORY: Record<string, string> = {
  general: "India", society: "Local", science_technology: "Technology", politics_government: "India",
  economy_business_finance: "Business", arts_culture_entertainment: "Entertainment", sport: "Sports", health: "Health",
};
type CurrentsArticle = { id?: string; title?: string; description?: string; url?: string; image?: string; published?: string; category?: string[] };
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!response.ok) return Response.json({ configured: true, stories: [], error: payload.message || payload.msg || `Currents returned ${response.status}` }, { status: 502, headers: { "Cache-Control": "no-store" } });
    ...
    return Response.json({ configured: true, stories, fetchedAt: new Date().toISOString() }, { headers: { "Cache-Control": "public, max-age=180, s-maxage=900, stale-while-revalidate=1800" } });
  } catch {
